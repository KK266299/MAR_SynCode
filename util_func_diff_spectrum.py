...
def marBHC(proj, metalBW,ray_trafo, FBPOper, param):
  ...
  #  projMetal = np.asarray(ray_trafo(metalBW)/param.reso)
    projMetal = np.asarray(ray_trafo(metalBW))
...
def get_mar_params(param_root, spectrummatfile, kvptube, photonmat):
    # materials
    ...
    a = sio.loadmat(param_root + '/'+ spectrummatfile)
    keyword = spectrummatfile.split('.')[0]
    GE14Spectrum120KVP=a[keyword]
    ...
    photonfile =  sio.loadmat(param_root+'/'+photonmat)
    keyword = photonmat.split('.')[0]
    photon=photonfile[keyword]
   # photonNum = 2*10**7
    photonNum = sum(photon)
   ...
   # spectrum = GE14Spectrum120KVP[:kVp, 1]
    spectrum = GE14Spectrum120KVP[:kVp]
    # water BHC
